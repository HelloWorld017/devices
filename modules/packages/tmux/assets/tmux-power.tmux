...
case $TC in
    ...
    'everforest' )
        ...
        ;;
    'custom' )
        TC=$(tmux_get @tmux_power_tc "#ffb86c")
        ;;
esac

T0=$(tmux_get @tmux_power_t0 "#ffffff")
T1=$(tmux_get @tmux_power_t1 "#a0a0a0")
T2=$(tmux_get @tmux_power_t2 "#202020")

G0=$(tmux_get @tmux_power_g0 "default")
G1=$(tmux_get @tmux_power_g1 "#101010")
G2=$(tmux_get @tmux_power_g2 "#303030")
G3=$(tmux_get @tmux_power_g3 "#3a3a3a")
G4=$(tmux_get @tmux_power_g4 "#444444")
G5=$(tmux_get @tmux_power_g5 "#e0e0e0")
...
if "$show_user" && "$show_host"; then
    LS="#[fg=$TC,bg=$G0]$larrow#[fg=$T2,bg=$TC,bold]$user_icon $(whoami)@#h #[fg=$TC,bg=$G0,nobold]$rarrow "
elif "$show_user"; then
    LS="#[fg=$TC,bg=$G0]$larrow#[fg=$T2,bg=$TC,bold]$user_icon $(whoami) #[fg=$TC,bg=$G0,nobold]$rarrow "
elif "$show_host"; then
    LS="#[fg=$TC,bg=$G0]$larrow#[fg=$T2,bg=$TC,bold] #h #[fg=$TC,bg=$G0,nobold]$rarrow "
fi

# session
if "$show_session"; then
    LS="$LS#[fg=$G1,bg=$G0]$larrow#[fg=$T0,bg=$G1]$session_icon #S #[fg=$G1,bg=$G0]$rarrow "
fi

# upload speed
if "$show_upload_speed"; then
    LS="$LS#[fg=$G1,bg=$G0]$larrow#[fg=$T0,bg=$G1]$upload_speed_icon #{upload_speed} #[fg=$G1,bg=$G0]$rarrow "
fi
...
RS="#[fg=$G3,bg=$G0]$larrow#[fg=$T0,bg=$G3]$time_icon $time_format #[fg=$G3,bg=$G0]$rarrow #[fg=$G1,bg=$G0]$larrow#[fg=$T0,bg=$G1]$date_icon $date_format #[fg=$G1,bg=$G0]$rarrow"
if "$show_download_speed"; then
    RS="#[fg=$G2,bg=$G0]$larrow#[fg=$T0,bg=$G2]$download_speed_icon #{download_speed} #[fg=$G2,bg=$G0]$rarrow $RS"
fi
...
tmux_set window-status-current-format "#[fg=$G5,bg=$G0]$larrow#[fg=$TC,bg=$G5,bold] #I:#W#F #[fg=$G5,bg=$G0,nobold]$rarrow"
